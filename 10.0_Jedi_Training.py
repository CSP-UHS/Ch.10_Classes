...
class Animal():
    def __init__(self, name):  # init is a constructor (you don't have to call it, it already runs)
...
"""
 2. Write code that defines a class named Cat:
     ...
     * Modify the constructor, so it calls the parent constructor as well. super
...
class Cat(Animal):
    ...
    def noise(self):
        print("Meow, says " + self.name)  # will be the cat self.name not the animal self.name
...
class Dog(Animal):
    ...
    def noise(self):
        print("Bark, says " + self.name)  # will be the dog self.name not the animal or cat self.name
